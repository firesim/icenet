 ...
 */
object IceNetConsts {
  val PROC_SPEED = 3.2 // assumed freq. in GHz

  // ethernet constants given by IEEE standard
...
case class IceNetConfig(
  ...
  def NET_IF_WIDTH_BYTES: Int = NET_IF_WIDTH_BITS / 8 // flit size in bytes
  def NET_FULL_KEEP = ~0.U(NET_IF_WIDTH_BYTES.W) // bytemask indicating which bytes should be kept

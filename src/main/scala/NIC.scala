...
class IceNicRecvPathModule(outer: IceNicRecvPath)
  ...
  val dropless = creditTrackerParams.nonEmpty || usePauser

  val buffer = Module(new NetworkPacketBuffer(
    inBufFlits, dropChecks = dropChecks, dropless = dropless))
...
trait HasPeripheryIceNICModuleImp extends LazyModuleImp {
  ...
  def connectNicLoopback(qDepth: Int = 4 * packetWords, latency: Int = 10) {
    ...
    if (creditTrackerParams.nonEmpty) {
      ...
      tracker.io.ext.flipConnect(NetDelay(net, latency))
      buffer.io.stream.in <> tracker.io.int.in
      tracker.io.int.out <> buffer.io.stream.out
      tracker.io.in_free := buffer.io.free
    } else if (p(NICKey).usePauser) {
